...
| Setting | Example |
| ------- | ------- |
| LAN IP | `192.168.1.9` (DHCP reservation on router preferred) |
...
**Status:** Documented below as the **target setup**. Cluster works locally on the Mac Mini (`ssh devlabs@192.168.1.9` → `kubectl get nodes`), but remote control from the MacBook via SSH tunnel is not verified yet.
...
```bash
mkdir -p ~/.kube
scp devlabs@192.168.1.9:~/.kube/config ~/.kube/devlabs-mini-config
```
...
ssh devlabs-mini 'bash -lc "MAC_MINI_IP=192.168.1.9 ~/spark-platform/scripts/deploy.sh"'
```
...
MacBook (dev machine)
    ...
    ▼
Mac Mini M4 — devlabs@192.168.1.9
    │
    ├── macOS 26.x
...
- [ ] DHCP reservation for `192.168.1.9`
...
| http://192.168.1.9:30080 | Spark History Server (completed jobs) |
...
| URL | Purpose |
|-----|---------|
| http://192.168.1.9:30900 | S3 API |
| http://192.168.1.9:30901 | MinIO web console |

Deploy or upgrade on the Mac Mini:

```bash
MAC_MINI_IP=192.168.1.9 ~/minio-platform/scripts/deploy.sh
```
...
| URL | Purpose |
|-----|---------|
| http://192.168.1.9:30089 | Job portal (trigger DAGs, runs, task logs) |
| http://192.168.1.9:30081 | Native Airflow UI (`admin` / `admin`) |

Deploy or upgrade on the Mac Mini:

```bash
MAC_MINI_IP=192.168.1.9 ~/airflow-platform/scripts/deploy.sh
```
...
|------------------|---------|
| `postgresql://devlabs@192.168.1.9:30432/devlabs` | LAN access (NodePort **30432**) |
| `postgres.postgres.svc.cluster.local:5432` | In-cluster |

Deploy or upgrade on the Mac Mini:

```bash
MAC_MINI_IP=192.168.1.9 ~/postgres-platform/scripts/deploy.sh
```
...
| URL | Purpose |
|-----|---------|
| http://192.168.1.9:30090 | Unified platform status, component health, CPU/RAM allocations |

Deploy or upgrade:

```bash
MAC_MINI_IP=192.168.1.9 ~/devlabs-dashboard/scripts/deploy.sh
```
