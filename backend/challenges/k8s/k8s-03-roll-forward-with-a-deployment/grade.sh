#!/usr/bin/env bash
# Grade k8s-03-roll-forward-with-a-deployment — Order Processor Deployment.
set -euo pipefail
...
if kubectl -n "$LEARNER_NS" get rs "$DEPLOY" >/dev/null 2>&1 && ! kubectl -n "$LEARNER_NS" get deploy "$DEPLOY" >/dev/null 2>&1; then
  fail "found ReplicaSet/${DEPLOY} without a Deployment — this lab requires a Deployment"
fi
...
SEL_APP="$(kubectl -n "$LEARNER_NS" get deploy "$DEPLOY" -o jsonpath='{.spec.selector.matchLabels.app}' 2>/dev/null || true)"
SEL_VER="$(kubectl -n "$LEARNER_NS" get deploy "$DEPLOY" -o jsonpath='{.spec.selector.matchLabels.version}' 2>/dev/null || true)"
...
if [[ -z "$PORTS" ]]; then
  fail "containerPort 8000 must be declared on the container (got no ports)"
fi
FOUND_PORT=0
for p in $PORTS; do
  if [[ "$p" == "8000" ]]; then
...
[[ "$FOUND_PORT" -eq 1 ]] || fail "containerPort must include 8000 (got '${PORTS}')"
...
pass "deployment/${DEPLOY} has ${WANT_REPLICAS} Ready pods in ${LEARNER_NS} with required image, labels, port, and resources"
