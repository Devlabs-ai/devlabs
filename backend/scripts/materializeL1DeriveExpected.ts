 ...
 *
 *   cd backend && npx tsx scripts/materializeL1DeriveExpected.ts
 */
...
const PREFIX = 'challenges/l1-derive-revenue-column';
const MAIN = `s3a://${BUCKET}/${PREFIX}/solution/src/main.py`;

const CASES = [
  { id: '01-run-edges', label: 'Run' },
  { id: '02-submit-all', label: 'Submit' },
] as const;
...
async function runCase(
  ...
  const k8sName = `ref-l1drv-${spec.id.slice(0, 8)}-${stamp}`.replace(/[^a-z0-9-]/g, '').slice(0, 52);
  ...
  const body = {
    ...
    spark_conf: { 'spark.sql.adaptive.enabled': 'false' },
    env: {
