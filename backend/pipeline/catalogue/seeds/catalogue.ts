...
const CATALOG = [
...

  {
    ...
    details: {
      ...
      port: 9042,
      imageHints: ['cassandra:5', 'cassandra:4.1'],
      defaultUser: 'cassandra',
      defaultDb: 'system',
    },
    text: 'Docker image `cassandra:latest` is the canonical Apache Cassandra tag for compose sandboxes. CQL native port 9042; in-network `cassandra:9042`. Optional env: `CASSANDRA_CLUSTER_NAME`, `CASSANDRA_DC`, `CASSANDRA_RACK`, `MAX_HEAP_SIZE`, `HEAP_NEWSIZE`. Init CQL scripts in `/docker-entrypoint-initdb.d/` run on first boot only. Healthcheck: `cqlsh -e "DESCRIBE KEYSPACES"` or `nodetool status` — allow 60–90s on first start. Python: `cassandra-driver` (`Cluster(["cassandra"])`, port 9042). Pitfalls: dependents must wait for CQL readiness; single-node keyspaces need `replication_factor: 1`; do not use Bitnami Cassandra tags.',
    defaultLimits: { cpus: '1.0', memory: '1G' },
  },
