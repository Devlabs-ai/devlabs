...
export const STRUCTURED_CHAT_TAGS = [
  'shape_contract',
  'challenge_draft',
  'problem_statement',
  'shape_meta',
...
/** Remove structured agent tags (and unclosed trailing blocks while streaming). */
...
export function hasStructuredChatTags(text: string): boolean {
  return STRUCTURED_CHAT_TAGS.some((tag) => new RegExp(`<${escapeRegex(tag)}>`, 'i').test(text || ''));
}

function footnoteForTags(raw: string): string | null {
  if (/<challenge_draft>/i.test(raw)) {
    return 'Schema materialized — see Preview panel.';
  }
  if (/<shape_contract>/i.test(raw)) {
    return 'Design contract updated — see Preview panel.';
  }
  if (hasStructuredChatTags(raw)) {
    return 'Structured output updated — see Preview panel.';
  }
  return null;
}

/** Final assistant message stored in chat history and shown after streaming completes. */
...
/** Incrementally hide structured tags while SSE text deltas arrive. */
export class ChatDisplayStreamFilter {
  ...
  finalize(): string {
    return formatChatForDisplay(this.raw);
  }
...
module.exports = {
  STRUCTURED_CHAT_TAGS,
  stripStructuredChatTags,
  hasStructuredChatTags,
