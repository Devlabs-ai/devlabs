...
function makeKeystrokeReconstructor({ onCommand }: { onCommand: (cmd: string) => void }): (data: unknown) => void {
  let pending = '';
  let inEscape = false;
  let escapeBuf = '';

  return function feed(data: unknown): void {
...
function handleConnection(ws: ObservabilityWebSocket, req: IncomingMessage): void {
  ...
  let lastEntry: { cmd: string; output: string } | null = null;
  ...
  const onStdin = ({ sessionId: sid, data }: { sessionId: string; data: unknown }) => {
    if (sid !== sessionId) return;
    feed(data);
  };

  const onStdout = ({ sessionId: sid, data }: { sessionId: string; data: unknown }) => {
    if (sid !== sessionId) return;
    if (!lastEntry) return;
    if (outputCharsSinceCommand >= MAX_OUTPUT_CHARS) return;
    const text = (Buffer.isBuffer(data) ? data : Buffer.from(data as string)).toString('utf8');
  ...
  const onSessionEnd = ({ sessionId: sid }: { sessionId: string }) => {
    if (sid !== sessionId) return;
