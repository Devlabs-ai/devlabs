...
function stripDockerLogFrames(buf) {
...
function handleConnection(ws, req) {
  ...
  const service = session.metricsService;
  if (!service) {
  ...
  const handleLine = (line) => {
    ...
    if (!current.recovered && current.recoveryCounter >= RECOVERY_REQUIRED) {
      ...
      sessionStore.persistRow(current).catch((err) =>
  ...
  const onData = (raw) => {
    const stripped = stripDockerLogFrames(Buffer.isBuffer(raw) ? raw : Buffer.from(raw));
  ...
  bus.on('session_end', function onEnd(payload) {
