...
function previewWsUrls(sessionId: string): { terminalWsUrl: string; metricsWsUrl: string } {
  return {
...
function publicPreviewSession(session: Record<string, unknown>, challenge: unknown): Record<string, unknown> {
  const services = ((session.services as string[]) || []).filter((s: string) => !/^load[-_]?gen/i.test(s));
  return {
    id: session.id,
...
async function endPreviewForReview(reviewSessionId: string): Promise<void> {
...
router.get('/', async (_req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
...
router.get('/:sessionId', async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
...
router.post('/:sessionId/preview', async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
...
router.post('/:sessionId/preview/end', async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
    const review = await reviewStore.get(req.params.sessionId);
    if (!review) return res.status(404).json({ error: 'review not found' });
    await endPreviewForReview(req.params.sessionId);
...
router.post('/:sessionId/push', async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
    const review = await reviewStore.get(req.params.sessionId);
    if (!review) return res.status(404).json({ error: 'review not found' });
...
router.delete('/:sessionId', async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
    await endPreviewForReview(req.params.sessionId);
...
function normalizeFeedbackTags(raw: unknown): string[] {
...
async function ensureDraftFromReview(review: Record<string, unknown>, sessionId: string): Promise<Record<string, unknown>> {
...
router.post('/:sessionId/send-back', async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
    ...
    const severity = body?.severity === 'suggestion' ? 'suggestion' : 'blocker';
    const tags = normalizeFeedbackTags(body?.tags);
