...
router.post('/request-otp', async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
    const email = ((req.body as Record<string, string>)?.email || '').toLowerCase().trim();
...
router.post('/verify-otp', async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
...
router.post('/companies', requireAdmin, async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
    const { name, domain, plan, subscriptionEnd } = (req.body as Record<string, unknown>) || {};
    ...
    if ((e as NodeJS.ErrnoException).code === '23505') return res.status(409).json({ error: 'Domain already registered' });
    next(e);
  }
});
...
router.post('/invites', requireInterviewer, async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
    const body = (req.body as Record<string, unknown>) || {};
    const { challengeId, name, email } = body;
...
router.get('/invites', requireInterviewer, async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
...
router.get('/invite/:token', async (req: ExpressRequest, res: ExpressResponse, next: ExpressNextFunction) => {
  try {
    const invite = await invites.resolveInvite(req.params.token);
    if (!invite) return res.status(404).json({ error: 'invite not found' });
    if ((invite as Record<string, unknown>).expired) {
      return res.status(410).json({ error: 'This invite link has expired' });
    }
    res.json({ name: (invite as Record<string, unknown>).name, challengeId: (invite as Record<string, unknown>).challengeId, used: (invite as Record<string, unknown>).used });
  } catch (e) {
    next(e);
  }
});
