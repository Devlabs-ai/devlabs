...
Clusters: Card Rails (transactional) and NovaMart (clickstream).
MinIO: datasets/payment-network/ and datasets/novamart/
Nothing is pre-selected: pick TXNS_* / EVENTS_* / DIM_* yourself, set Resources, then Run.
...
def main() -> None:
    ...
    # txns = spark.read.parquet(TXNS_10K)
    # txns = spark.read.parquet(TXNS_100K)
...
export function buildSparkPlaygroundChallenge(): ChallengeFull {
  return {
    ...
    difficulty: 'Practice',
    tags: ['spark', 'playground', 'practice', 'transactional', 'card-rails', 'clickstream', 'novamart'],
