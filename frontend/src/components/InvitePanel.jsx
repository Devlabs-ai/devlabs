...
import { createInvite, fetchInvites } from '../services/authApi.js';

export default function InvitePanel({ challenges }) {
  ...
  const handleCreate = async () => {
    ...
    setError(null);
    try {
      await createInvite({ challengeId: selectedChallenge || null });
      await reload();
    } catch (e) {
      setError(e?.response?.data?.error || e.message);
    } finally {
