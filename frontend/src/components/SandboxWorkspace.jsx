...
import MetricsDashboard, { useMetricsState } from './MetricsDashboard.jsx';

function useMetricsStream(wsUrl, external) {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (ev) => {
      try { internal.handleMessage(JSON.parse(ev.data)); } catch (_e) { /* ignore */ }
    };
          ...
          <div className="panel-body flush">
            ...
            <div style={{ display: rightTab === 'editor' ? 'flex' : 'none', flexDirection: 'column', flex: 1, minHeight: 0 }}>
              <CodeEditor
                ...
                services={session.services}
                defaultContainer={session.terminalService}
              />
            </div>
