...
function withContainer(baseUrl: string, container: string): string {
  try {
...
export default function TerminalWorkspace({ services, baseWsUrl, defaultService }: TerminalWorkspaceProps): JSX.Element {
  const initial = defaultService || services?.[0] || null;
  const [active, setActive] = useState<string | null>(initial);
  const [opened, setOpened] = useState<Set<string>>(() => (initial ? new Set([initial]) : new Set()));
  ...
  const handleSelect = (svc: string): void => {
    setActive(svc);
    setOpened((prev) => {
