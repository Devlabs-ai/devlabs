...
export function serviceNameFromPortKey(key) {
  return key.replace(/^HOST_PORT_/, '').toLowerCase().replace(/_/g, '-');
}

export function servicesFromPortMap(portMap) {
...
export function pickDefaultBrowseService(portMap) {
...
export function directServiceUrl(port, host = sandboxHost()) {
  return `http://${host}:${port}`;
}

/** Same-origin path served by the backend browse proxy (embeddable in iframe). */
export function proxiedBrowseUrl(sessionId, service) {
...
export function displayBrowseUrl(sessionId, service) {
...
export function isMixedContentBlocked(url) {
...
export function isBrowseProxyErrorPayload(text, contentType = '') {
  const t = (text || '').trim();
...
export function normalizeBrowseUrl(url) {
